class Api::V1::AppointmentsController < ApplicationController
  ...
  def index
    appointments = current_user.appointments.includes(:provider, :client)

    # Apply filtering
    appointments = filter_appointments(appointments)

    render json: AppointmentSerializer.new(appointments, include: [:provider, :client]).serializable_hash
  end
  ...
  def show
    ...
    render json: AppointmentSerializer.new(@appointment, include: [:provider, :client]).serializable_hash
  end
